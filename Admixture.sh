...
Refs=${inPATH}Input/RefsPlink.txt
...
do
	...
	echo "$N"
	# echo $N
	${AdmPath} --supervised ${inPATH}Analysis/samplesAnalyse${2}.bed 6 -B100
	sed -n "$N""p" ${inPATH}samplesAnalyse${2}.6.Q >> $outQ
	echo "$S" ' ' "$N" ' ' "$a" >> $outNames
	#sed -i "$Ns/^/$S\/" $outQ
	sed -n "$N""p" ${inPATH}samplesAnalyse${2}.6.Q_se >> $outCI
done < "$samples"
